...
module.exports = class ChordBox {
	...
	constructor(sel, params) {
		...
		this.canvas = SVG()
			.addTo(sel)
			.size(this.params.width + 5, this.params.height + 10);
	...
	draw({ chord, position, barres, positionText, positionTextX = 0, tuning, finger }) {
		...
		if (this.position <= 1) {
			...
			this.canvas
				.rect(
					this.x + spacing * (this.numStrings - 1) - fromX,
					this.y - fromY
				)
				.move(fromX, fromY)
				.stroke({ width: 0 })
			...
			this.drawText(
				this.x - this.spacing / 2 - this.spacing * 0.1 + positionTextX,
	...
	lightBar(stringFrom, stringTo, theFretNum) {
		...
		const y =
			this.y + this.fretSpacing * (fretNum - 1) + this.fretSpacing / 4;
		const yTo =
			this.y +
